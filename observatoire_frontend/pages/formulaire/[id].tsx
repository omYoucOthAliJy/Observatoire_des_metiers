...
export default function FormulaireQuestionnaire() {
    ...
    const submitMethod = async (data: any) => {
        ...
        const response = await FormulaireApi.updateFormulaire(cookieData.token, Number(router.query.id), payload);
        if(response.ok) {
            router.push('/acceuil')
        }else {
    ...
    useEffect(() => {
        ...
        if (currentUser) {
          ...
          FormulaireApi.getFormulaireById(cookieData.token, id as any).then((res) => {
            if (res.ok) {
